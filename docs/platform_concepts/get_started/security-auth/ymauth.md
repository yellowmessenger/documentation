...
Validation fails when the token expires due to customer inactivity. In such situations, a function must be called assigning the `refreshSession` value. Follow these steps:
...
1. Add an **event listener** on your platform to identify if the token gets expired in between the conversation of the AI-agent. Use:
    ...
    - **Data**: currentToken, refreshSession (true/false value stating if the AI-agent is currently in open state)
...
3. Pass the newly generated encryption token to the AI-agent by calling the below **function**.
    - `refreshSession` flag must be passed as false if you want the AI-agent to remain open and show the loading spinner while the input box is disabled. If it is not false, the AI-agent closes and opens again, which might confuse your customer.

    ```
    window.YellowMessengerPlugin.revalidateToken(token: string, refreshSession = true)
    ```
