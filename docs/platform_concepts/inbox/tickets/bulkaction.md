...
1. [How to change the status of multiple tickets and assign them to a group/agent?](#1)
2. [How to merge multiple tickets into one ticket?](#2)
...
To simplify the repetitive tasks performed by agents, bulk actions can be used by selecting multiple tickets and performing a common action on them. 
Tickets can be merged into one ticket and assigned to one agent. Merging tickets can avoid providing the same replies to multiple tickets, when multiple tickets are combined, the same response can be sent to all the customers. 
...
## <a name="1"></a>  1. Change status and transfer tickets in bulk 
...
4. Bulk action can be used to change the status of the tickets or/and to assign tickets in bulk to a particular group or agent.
5. You can deselect the tickets by clicking **Clear selection**.
...
## <a name="2"></a>  2. Merge multiple tickets into one ticket

Multiple tickets from a requestor can be merged and a single ticket, it can be resolved by one agent. To merge tickets, follow the steps below: 
