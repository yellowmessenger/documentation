...
4. Open the API or [add a new API](./add-api) to which you want to add access token (Authorization header).

> In the example, we will be adding a search lead API from Salesforce. Pass the bearer token in the header and email as a [parameter to this API](./send-data). So we will fetch values from [variables](../build/bot-variables) and pass it to the API using this syntax `{{{variable_name}}}`; as shown in the screenshots below.
...
6. Create an [API action node](../build/nodes/action-nodes#api) to invoke auth API and store the response in a custom variable.
